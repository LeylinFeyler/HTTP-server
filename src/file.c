...
void send_error_page(int client_fd, int status, const char *status_text, int keep_alive) {
    ...
    if (!file) {
        send_status_response(client_fd, status, status_text, status_text, keep_alive);
        return;
    }
    ...
    snprintf(header, sizeof(header),
             ...
             "Connection: %s\r\n"
             "\r\n",
             status, status_text, size, keep_alive ? "keep-alive" : "close");
...
void send_file(int client_fd, const char *path, int send_body, int keep_alive) {
    ...
    if (!is_safe_path(path)) {
        send_error_page(client_fd, 403, "Forbidden", keep_alive);
    ...
    if (!file) {
        send_error_page(client_fd, 404, "Not Found", keep_alive);
        return;
    }
    ...
    snprintf(header, sizeof(header),
             ...
             "Connection: %s\r\n"
             "\r\n",
             get_mime_type(full_path), size, keep_alive ? "keep-alive" : "close");
